...
def feed_htmx(request: Request, nextUrl: str):
    ...
    for image in feed["data"]:
        ...
        if image["media_type"] == "CAROUSEL_ALBUM":
            ...
            <div id="carousel-loader-{image["id"]}" class="loader-class" hidden>
                <div role="status">
                    <svg class="inline w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
            ...
            <script>
                document.getElementById("{childImage["id"]}").addEventListener('click', (e) => {{
                    ...
                    e.addEventListener('htmx:beforeRequest', function (evt) {{
                        ...
                        const loadingIndicator = document.querySelector(`#carousel-loader-${childImage["id"]}`);
